...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    const pdf = process.env.VERCEL ? await renderPdfWithPuppeteer(html) : await renderPdfWithPlaywright(html);
...
async function renderPdfWithPuppeteer(html: string) {
  ...
  const browser = await puppeteer.launch({
    ...
    executablePath: await chromiumRuntime.default.executablePath(),
    headless: true,
  });
